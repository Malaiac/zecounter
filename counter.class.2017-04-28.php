...
{ // BEGIN class counter
  ...
  foreach($this->timers as $name => $time)
    ...
    $count = $this->loops[$name];
    if($logger) {
     $path = explode('_',$name);
     $log = '';
     //$log = str_repeat("\t",1+count($path)).array_pop($path);
     $log = '['.implode('] [',$path).']';
					if($count > 1) $log .= " x $count";
					$log .= ' = '.round($time/1000,4);
					$logger->tip($log,'counter');
				 }
    $html .= '
    <tr class="'.$odd.'"><td>'.$name;
				if($count > 1) $html .= " (x$count)";
				$html .= '</td><td>'.round($time/1000,4).'</td></tr>';
    }
  if(isset($this->counters) && count($this->counters)) 
		foreach($this->counters as $name => $count)
    {
    if($time < 10) continue;
    if($logger) {
     $path = explode('_',$name);
					$log = str_repeat("\t",count($path)).array_pop($path)." x $count";
					$logger->tip($log,'counter');
				 }
    $odd = ($odd == 'odd') ? 'even' : 'odd';
    $html .= '
  ...
  function start($marker)
    ...
    if(!isset($this->loops[$marker])) $this->loops[$marker] = 1;
    else $this->loops[$marker]++; 
